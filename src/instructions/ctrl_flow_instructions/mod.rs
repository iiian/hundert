...
/// A control flow instruction. Returns the next instruction.
pub trait CtrlInstr<'i> {
    fn execute(&self, core: &mut Core) -> &'i Ctrl;
}

/// All supported instructions for the TIS-100 instruction set. Includes a wrapper for mutable
/// instructions which simply return the next instruction.
pub enum Ctrl<'i> {
    None,
    MutWrap(MutWrap<'i>),
    JCmp(JCmp<'i>),
